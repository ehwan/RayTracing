...
struct BoundingBox
{
  ...
  {
    return (min_.array()<b.max_.array()).all() && (b.min_.array()<max_.array()).all();
  }
  float area() const
  {
...
struct Triangle : GeometryObject
{
  ...
  Triangle( vec3 const& _p0, vec3 const& _p1, vec3 const& _p2,
            vec3 const& _n0, vec3 const& _n1, vec3 const& _n2 )
  ...
  */
  RayHit raycast( Ray const& r ) const override
  {
    ...
    if( std::abs(det) < EPSILON ){ return RayHit::no_hit(); }
    det = 1.0f / det;
